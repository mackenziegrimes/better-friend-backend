"""Business logic for /users Route"""
...
from src.firestore import User, Firestore
...
async def get_all_users() -> Optional[Sequence[User]]:
    """Fetch all Users db objects"""
    response: Sequence[DocumentReference] = Firestore.get_collection().list_documents()
...
async def create_user() -> Optional[User]:
    ...
    user_doc: DocumentReference = Firestore.get_collection().document()
...
async def get_user(user_id: str) -> Optional[User]:
    ...
    current_app.logger.debug(f"Now getting user id {user_id}")
    try:
        user = Firestore.get_collection().document(user_id).get().to_dict()
        ...
        current_app.logger.info(f"Failed to get user id {user_id}: {e}")
        return Response(status=400)


@bp.route("/<string:id>", methods=["PATCH"])
async def patch_user(id: str) -> Optional[User]:
    """Update one User object in db"""
    ...
    try:
        user_doc: DocumentReference = Firestore.get_collection().document(id)
...
async def delete_user(id: str) -> Optional[User]:
    ...
    try:
        ...
        user_doc: DocumentReference = Firestore().document(id)
        user_snapshot: DocumentSnapshot = user_doc.get()
